...
RValue CodeGenFunction::EmitCXXMemberOrOperatorMemberCallExpr(
  ...
  RValue rval = EmitCXXMemberOrOperatorCall(
                    CalleeDecl, Callee, ReturnValue, This.getPointer(),
    ...
    if (CI) {
      ...
      if (!ReceiverType->isRecordType()) {
        if (const PointerType* PT = ReceiverType->getAs<const PointerType>()) {
          ...
          llvm::errs() << "ERROR: receiver type is not a pointer type\n";
          DEBUG(ReceiverType->dump());
        }
      }
      if (const RecordType* RT = ReceiverType->getAs<const RecordType>()) {
        ...
        addSoaapVTableMetadata(CI, SRD, "static");
        staticTypeFound = true;
      }

      if (!staticTypeFound) {
        llvm::errs() << "ERROR: cannot obtain the static type of the implicit object argument!\n";
        DEBUG(CE->getLocStart().print(llvm::dbgs(), getContext().getSourceManager()));
        DEBUG(CI->dump());
        DEBUG(Receiver->dump());
      }
    }
    else {
      llvm::errs() << "ERROR: no CallInst was found from CXXMemberCallExpr\n";
      DEBUG(CE->getLocStart().print(llvm::dbgs(), getContext().getSourceManager()));
      DEBUG(CE->dump());
    }
  }
